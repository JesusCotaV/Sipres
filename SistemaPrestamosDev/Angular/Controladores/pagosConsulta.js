(function () {
    ...
    angular.module("myApp").controller('pagosConCtrl', pagosConCtrl);
    pagosConCtrl.$inject = ['$scope', '$rootScope', "commonFactory", "$firebaseArray","$timeout","$location"];
    //Inicio Funcion controlador
    function pagosConCtrl($scope, $rootScope, common, $firebaseArray,$timeout,$location) {
        var ctrl = this;
        ////Variables////
        var ref = firebase.database().ref("/Pagos");
        $scope.ListadoPagos = $firebaseArray(ref);
        console.log($scope.ListadoPagos);
        //////funciones//////
        ...
        function removeproducto(cliente) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $scope.ListadoPagos.$remove(cliente);
                }
              })
